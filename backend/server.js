...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    // ডুপ্লিকেট প্রতিরোধ করে ডক্টরস টেবিলে সিঙ্ক
...
app.get('/api/users/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await pool.query('SELECT id, name, email, role, specialty, license, phone, bio, image_url, hospital, fee, age, gender, blood_group, height, weight FROM users WHERE id = $1', [id]);
  ...
  try {
    ...
    try {
      await pool.query(`
        ...
        WHERE user_id = $6
      `, [rows[0].name, rows[0].image_url, rows[0].hospital, rows[0].fee, rows[0].specialty, parsedId]);
    } catch (syncErr) {
      console.warn('Doctor sync skipped:', syncErr.message);
    }
...
// DISTINCT ON ব্যবহার করে শতভাগ ডুপ্লিকেট মুক্ত ডক্টরস রেসপন্স
...
app.post('/appointments', async (req, res) => {
  try {
    const { patient_id, patient_name, doctor_id, doctor_name, specialty, date, time } = req.body;
    ...
    const query = `
      INSERT INTO appointments (patient_id, patient_name, doctor_id, doctor_name, specialty, date, time, status)
...
app.post('/prescriptions', async (req, res) => {
  try {
    const { doctor_id, doctor_name, patient_id, patient_name, diagnosis, medicines, instructions, date } = req.body;
    if (!doctor_name || !patient_name || !medicines) {
      return res.status(400).json({ error: 'Missing required prescription fields' });
    }
    const query = `
      INSERT INTO prescriptions (doctor_id, doctor_name, patient_id, patient_name, diagnosis, medicines, instructions, date)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
      RETURNING *;
    `;
    const values = [
      doctor_id ? doctor_id.toString() : '1',
      doctor_name,
      patient_id ? patient_id.toString() : 'P-101',
      patient_name,
    ...
    const { rows } = await pool.query(query, values);
    res.status(201).json({ message: 'Prescription created successfully', prescription: rows[0] });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
